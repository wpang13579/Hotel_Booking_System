...
if ($role != 4) {
    die("<p style='color: red;'>Access denied. Only managers can create maintenance tasks.</p>");
}
...
if (!$req_id) {
    die("<p style='color: red;'>Invalid maintenance request ID. Please ensure the req_id is passed in the URL.</p>");
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (mysqli_query($con, $insert_task_query)) {
        ...
        if (mysqli_query($con, $update_request_query)) {
            echo "<script>
                alert('Task created successfully, and the request status has been updated to Approved.');
                window.location.href = 'viewMainTask.php';
            </script>";
            exit();
        } else {
            echo "<p style='color: red;'>Task created, but failed to update the request status: " . mysqli_error($con) . "</p>";
        }
    } else {
        echo "<p style='color: red;'>Error creating maintenance task: " . mysqli_error($con) . "</p>";
    }
}
?>
...
<body>
    <div class="container">
        ...
        <div class="btn-back-container">
            <a href="../maintenance request/viewMainRequest.php" class="btn-back">Back to View</a>
        </div>
    </div>
